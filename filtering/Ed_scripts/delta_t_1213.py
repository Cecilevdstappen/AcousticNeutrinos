...
def main(argv):
    ...
    for energy in np.arange(11,13,1):
        for zpos in np.arange(0,20,0.1):
            ...
            for i in range(0,1000):
                ...
                trace_start = random_start #pow(2,13), np.random.randint(0,len(data_array) - 1000, 1)[0]
                ...
                Y = np.fft.fft(y)
                m = np.abs(Y)
                phase = 2*pi*random_nr[i]*(len(m)) - pi
        ...
        #        template_array = signal.lfilter(frf.num, frf.dnum, template_array)
            ...
            aFile=open('../Neutrino_data_files/deltat_SNR_'+str(rpos)+'_'+str(round(float(zpos),1))+'_'+str(energy)+'.dat', 'wb')
