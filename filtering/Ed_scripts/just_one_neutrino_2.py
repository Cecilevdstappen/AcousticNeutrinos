...
#    np.random.seed(scaling)
    ...
    neutrino_noise_array = x+z 
    neutrino_noise_array += np.mean(y) * np.ones(len(z))     # need to apply a high pass filter , fixme !
...
import os
def main(argv):
    for zpos in np.arange(10,110,10): 
#    ypos = 300
...
#    commandlinestring = 'octave -q one_pulse.m ' + str(zpos) + \
    ...
    #    os.system(commandlinestring)
            filename = 'neutrino_' + str(zpos) +'_300_1_11.dat'
