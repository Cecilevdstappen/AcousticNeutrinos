...
def main(argv):
    ...
    L_delta_t = []
    L_max_snr = []
    random_nr = np.random.rand(10)
    for energy in np.arange(11,12,1):
    #for zpos in np.arange(0,20,0.1):
        for i in range(0,10):
            ...
            template_array = datafile(template_file)#to mPa
            ...
            print(random_start)
            trace_start = random_start#np.random.randint(0,len(data_array) - 1000, 1)[0]
            ...
            z = butter_highpass_filter(z, 1000, sampling_rate, order=5)
    ...
    #        template_array = signal.lfilter(frf.num, frf.dnum, template_array)
        ...
        #    
            ...
            #template = template.cyclic_time_shift(+0.0073)
            ...
            delta_t = data.sample_times[entry_point] - peak_t + 0.006 # 0.006 s is starting time of template
...
#        aFile=open('../Neutrino_data_files/delta_t_test.dat', 'wb')
#        aFile.write(pickle.dumps({"delta_t":L_delta_t,"snr":L_max_snr}))
#        aFile.close()
#        
        print(data.sample_times[entry_point],peak_t)
